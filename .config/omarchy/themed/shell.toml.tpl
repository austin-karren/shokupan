...
# ADR-0009: pinned, not the theme's bg variable. A fixed Tailwind-950 in
# every theme, light or dark, so it reads as system chrome instead of as part
# of the theme. Transparency is a separate axis and still toggles freely.
background       = "#0b0c14"
background-alpha = 1.0
# ADR-0009: pinned, not the theme's fg variable. A light theme's fg is dark —
# Catppuccin
# Latte's is #4c4f69 — and would be invisible on a near-black bar. The template
# engine does substitution only, with no conditionals, and no single variable is
# readable in both modes, so this is a literal. colors.toml now carries
# mode = "light", so a future generator with conditionals could restore the old
# behaviour of tinting light-theme text with the theme's own background.
text             = "#c0caf5"
