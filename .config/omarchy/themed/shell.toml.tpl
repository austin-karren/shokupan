...
#
# SHOKUPAN OVERRIDE (ADR-0009). A verbatim copy of
# /usr/share/omarchy/default/themed/shell.toml.tpl with two lines changed, both
# in [bar]. User templates in ~/.config/omarchy/themed/ take priority over the
# built-in ones, so this is the sanctioned extension point rather than a patched
# Omarchy file. It REPLACES the built-in template wholesale, so it does not
# inherit upstream changes — re-diff it against the default after an upgrade.

[bar]
# Alpha companions (where present) range from 0 (invisible) to 1 (opaque).
# ADR-0009: pinned, not the theme's background variable. A fixed Tailwind-950
# in every theme, light or dark, so it reads as system chrome instead of as
# part of the theme. Transparency is a separate axis and still toggles freely.
background       = "#0b0c14"
background-alpha = 1.0
# ADR-0009: pinned, not the theme's foreground variable. A light theme's
# foreground is dark — Catppuccin Latte's is #4c4f69 — and would be invisible
# on a near-black bar. The template engine does substitution only, with no
# conditionals, and no single variable is readable in both modes, so this is a
# literal. colors.toml carries mode = "light", so a future generator with
# conditionals could restore the old behaviour of tinting light-theme text
# with the theme's own background.
text             = "#c0caf5"
