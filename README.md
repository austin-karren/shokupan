...
| `.config/omarchy/plugins/shokupan-{calendar,omenu,apexshot}/` | Bar-widget plugins, converted from QML modules in ADR-0044's wave 1: the menu button wearing the power glyph and the ApexShot screenshot button (expected to be replaced by quattro's full-release screenshot tool), enabled by their `shokupan.*` ids in `shell.json`'s bar layout. The calendar bracket is DEPRECATED and off the bar since 2026-08-15 — the clock clone's date clicks replaced it (ADR-0006 addendum); the plugin dir stays as the marked tombstone |
