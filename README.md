...
| `.config/omarchy/plugins/shokupan-{omenu,capture}/` | Bar-widget plugins: the menu button wearing the power glyph (converted from a QML module in ADR-0044's wave 1) and the capture button — same three clicks as the deprecated ApexShot button it replaced, on the native `omarchy-capture-*` tools (ADR-0044 addendum). Enabled by their `shokupan.*` ids in `shell.json`'s bar layout. `shokupan-apexshot/` and `shokupan-calendar/` remain as marked-deprecated tombstones — the calendar bracket is off the bar since 2026-08-15, replaced by the clock clone's date clicks (ADR-0006 addendum) |
| `.config/omarchy/plugins/austinkarren.clock/` | Clone of upstream's clock plugin via the sanctioned `omarchy plugin clone` shape (`clonedFrom: omarchy.clock` routes the bar entry and IPC automatically). Panel.qml is patched — marked `// SHOKUPAN:` — so day cells and the hero date open GNOME Calendar through `calendar-toggle --date` (ADR-0006 addendum); the other files are verbatim copies, re-diffed via `packages/forks` at upgrades |
