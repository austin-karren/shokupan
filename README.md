...
| `.config/omarchy/plugins/shokupan-{calendar,omenu,capture}/` | Bar-widget plugins: the static calendar button and the menu button wearing the power glyph (converted from QML modules in ADR-0044's wave 1), plus the capture button — same three clicks as the deprecated ApexShot button it replaced, on the native `omarchy-capture-*` tools (ADR-0044 addendum). Enabled by their `shokupan.*` ids in `shell.json`'s bar layout. `shokupan-apexshot/` remains as a marked-deprecated tombstone |
