...
| `.config/wireplumber/wireplumber.conf.d/` | One `zz-`named drop-in that removes A2DP from `bluez5.auto-connect` on mic-bearing devices, so nothing forces an A2DP stream back while a call holds the mic (ADR-0045 + addendum). Omarchy's A2DP-only fragment lives in the same real directory; `conf.d` fragments merge in filename order and the later matching rule wins, so this overrides without touching Omarchy's file |
